...
QCPGraphCurve::QCPGraphCurve(QCPAxis *keyAxis, QCPAxis *valueAxis):
QCPAbstractPlottable(keyAxis, valueAxis)
{
    mData = new QList<EigenData>();
...
QCPGraphCurve::~QCPGraphCurve()
{
    delete mData;
}

void QCPGraphCurve::setData(const QVector<EigenData> &value)
{
  mData->clear();
  for (int i=0; i<value.count(); ++i)
    mData->append(value[i]);
}

void QCPGraphCurve::setScatterStyle(QCP::ScatterStyle style)
{
  mScatterStyle = style;
}

void QCPGraphCurve::setScatterSize(double size)
{
  mScatterSize = size;
}

void QCPGraphCurve::setScatterPixmap(const QPixmap &pixmap)
{
  mScatterPixmap = pixmap;
}

void QCPGraphCurve::setLineStyle(QCPGraphCurve::LineStyle style)
{
  mLineStyle = style;
}

void QCPGraphCurve::addData(const EigenData &data)
{
  mData->append(data);
}

void QCPGraphCurve::clearData()
{
  mData->clear();
}

/* inherits documentation from base class */
double QCPGraphCurve::selectTest(const QPointF &pos) const
{
  if (mData->isEmpty() || !mVisible)
    return -1;
...
/* inherits documentation from base class */
void QCPGraphCurve::draw(QCPPainter *painter)
{
  if (mData->isEmpty()) return;
...
*/
void QCPGraphCurve::drawScatterPlot(QCPPainter *painter, const QVector<QPointF> *pointData) const
{
  // draw scatter point symbols:
  applyScattersAntialiasingHint(painter);
  painter->setPen(mainPen());
...
/*! \internal
  ...
  way.
*/
void QCPGraphCurve::getCurveData(QVector<QPointF> *lineData) const
{
  ...
  */
  ...
  for(int i = 0; i < mData->count(); ++i)//it = mData->constBegin(); it != mData->constEnd(); ++it)
  {
    x = (*mData)[i].x;
    y = (*mData)[i].y;
    ...
    */
    ...
    if (currentRegion == 5 || (firstPoint && mBrush.style() != Qt::NoBrush)) // current is in R, add current and last if it wasn't added already
    {
      if (!addedLastAlready) // in case curve just entered R, make sure the last point outside R is also drawn correctly
        lineData->append(coordsToPixels((*mData)[i-1].x, (*mData)[i-1].y)); // add last point to vector
      else if (lastRegion != 5) // added last already. If that's the case, we probably added it at optimized position. So go back and make sure it's at original position (else the angle changes under which this segment enters R)
      {
        if (!firstPoint) // because on firstPoint, currentRegion is 5 and addedLastAlready is true, although there is no last point
          lineData->replace(lineData->size()-1, coordsToPixels((*mData)[i-1].x, (*mData)[i-1].y));
      }
      lineData->append(coordsToPixels((*mData)[i].x, (*mData)[i].y)); // add current point to vector
    ...
    else if (currentRegion != lastRegion) // changed region, add current and last if not added already
    {
          ...
          )
      {
        ...
        if (!addedLastAlready)
          lineData->append(coordsToPixels((*mData)[i-1].x, (*mData)[i-1].y));
        // add current point, original:
        lineData->append(coordsToPixels((*mData)[i].x, (*mData)[i].y));
      }
      else // no special case that forbids optimized point placement, so do it:
      {
        // always add last point if not added already, optimized:
        if (!addedLastAlready)
          lineData->append(outsideCoordsToPixels((*mData)[i-1].x, (*mData)[i-1].y, currentRegion));
        // add current point, optimized:
        lineData->append(outsideCoordsToPixels((*mData)[i].x, (*mData)[i].y, currentRegion));
      }
  ...
  if (lastRegion != 5 && mBrush.style() != Qt::NoBrush && !mData->isEmpty())
    lineData->append(coordsToPixels((*mData)[mData->count()-1].x, (*mData)[mData->count()-1].y));
}
...
/*! \internal
  ...
  \ref selectTest.
*/
double QCPGraphCurve::pointDistance(const QPointF &pixelPoint) const
{
  if (mData->isEmpty())
  {
  ...
  if (mData->count() == 1)
  {
    QPointF dataPoint = coordsToPixels((*mData)[0].x,(*mData)[0].y);
    return QVector2D(dataPoint-pixelPoint).length();
  }
...
*/
QPointF QCPGraphCurve::outsideCoordsToPixels(double key, double value, int region) const
{
  int margin = 10;
...
/* inherits documentation from base class */
QCPRange QCPGraphCurve::getKeyRange(bool &validRange, SignDomain inSignDomain) const
{
  QCPRange range;
  bool haveLower = false;
  bool haveUpper = false;
...
/* inherits documentation from base class */
QCPRange QCPGraphCurve::getValueRange(bool &validRange, SignDomain inSignDomain) const
{
  QCPRange range;
  bool haveLower = false;
  bool haveUpper = false;
  ...
  int it = 0;//QCPCurveDataMap::const_iterator it = mData->constBegin();
  while (it != (*mData).count())
  {
    current = (*mData)[it].y;
    if (inSignDomain == sdBoth || (inSignDomain == sdNegative && current < 0) || (inSignDomain == sdPositive && current > 0))
...
void QCPGraphCurve::drawLegendIcon(QCPPainter *painter, const QRect &rect) const
{
  // draw fill:
  if (mBrush.style() != Qt::NoBrush)
  {
